...
public class TodoDAOImpl implements ITodoDAO {
	...
	public void insertTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = DBUtils.getConnection();
			...
			preparedStatement.setDate(4, todo.getTargetDate());
	...
	public Todo selectTodo(long todoId) {
		...
		try (Connection connection = DBUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Todo(id, title, username, description, Date.valueOf(targetDate), isDone);
			}
		} catch (SQLException exception) {
			DBUtils.printSQLException(exception);
		}
	...
	public List<Todo> selectAllTodos() {
		...
		try (Connection connection = DBUtils.getConnection();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Todo(id, title, username, description, Date.valueOf(targetDate), isDone));
			}
		} catch (SQLException exception) {
			DBUtils.printSQLException(exception);
		}
	...
	public boolean updateTodo(Todo todo) throws SQLException {
		...
		try (Connection connection = DBUtils.getConnection();
			...
			statement.setDate(4, todo.getTargetDate());
